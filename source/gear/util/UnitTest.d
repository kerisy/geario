...
string generateUnitTests(T)() {
    ...
    string str;
    str ~= `import std.stdio; import gear.logging.ConsoleLogger;
writeln("=================================");
    ...
    foreach (memberName; __traits(allMembers, T)) {
        ...
        static if (is(T == class) && FixedObjectMembers.canFind(memberName)) {
            ...
            static if (memberProtection == "private"
                ...
                // version (GEAR_DEBUG) pragma(msg, "skip private member: " ~ memberName);
            } else {
                    ...
                    static if(testWithUDAs.length >0) {
                        ...
                        static if(is(expectedType : Throwable)) {
                            str ~= "try { t." ~ memberName ~ "(); } catch(" ~ fullyQualifiedName!expectedType ~ 
                                " ex) { version(GEAR_DEBUG) { warning(ex.msg); } }\n";
                        } else {
                            str ~= "t." ~ memberName ~ "();\n"; 
                        }
 ...
 * {@link rules.ExpectedException ExpectedException} rule can be used. Further
 ...
 * <b>Consider using the {@link rules.Timeout} rule instead</b>, which ensures a test method is run on the
