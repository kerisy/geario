...
deprecated("It's buggy. Use gear.serialization.JsonSerializer instead.")
module gear.util.Serialize;
...
mixin template SerializationMember(T) {
    import std.traits;
    debug(GEAR_DEBUG_MORE) import gear.logging.ConsoleLogger;
    ...
    static if(baseClasses.length == 1 && is(baseClasses[0] == Object)) {
        ...
        override ubyte[] Serialize() {
            auto bytes = cast(ubyte[])gear.util.Serialize.serialize!(T, false)(this);
            debug(GEAR_DEBUG_MORE) 
            ...
            // auto bytes = cast(ubyte[])gear.util.Serialize.Serialize(this);
