...
module geario.util.Configuration;
...
class ConfigurationItem {
    ...
    @property ConfigurationItem SubItem(string name) {
        ...
        if (v is null) {
            ...
            log.warn("The items for '%s' is undefined! Use the defaults now", path);
        }
        return v;
    }
...
class ConfigBuilder {
    ...
    void SetValue(string key, string value) {
        ...
        foreach (str; list) {
            ...
            if (canFind(reservedWords, str)) {
                version (GEAR_CONFIG_DEBUG) log.warn("Found a reserved word: %s. It may cause some errors.", str);
            }
    ...
    private static string BuildSetFunction(T, string returnParameter, string incomingParameter)() {
        ...
        string str = "import geario.logging;";
                ...
                static if (is(memberType == interface)) {
                    ...
                    str ~= q{
                        if(%5$s.Exists("%1$s")) {
                            ...
                            version (GEAR_CONFIG_DEBUG) log.warn("Undefined item: %%s.%1$s" , %5$s.FullPath);
                        }                        
    ...
    private static SetClassMemeber(memberType, string settingItemName,
        ...
        string r = q{
            ...
            if(%6$s.Exists("%2$s")) {
                ...
                version (GEAR_CONFIG_DEBUG) log.warn("Undefined item: %%s.%2$s" , %6$s.FullPath);
            }
...
// version (unittest) {
//     import geario.util.Configuration;
